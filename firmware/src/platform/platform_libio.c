...
/* ------------------------------------------------------------------ */
/* ffconf.h is generated by CubeMX and is overwritten every time the   */
/* project is regenerated. These settings are not optional for the     */
/* index, and a silent revert costs a debugging session — so a         */
/* regeneration that resets them breaks the build instead.             */
/*                                                                     */
/* Set them in CubeMX (FATFS -> Advanced Settings), not only here.     */
/* ------------------------------------------------------------------ */

#if _FS_READONLY != 0
#error "ffconf.h: _FS_READONLY must be 0 — the library index writes /dap.idx"
#endif

#if _USE_WRITE != 1
#error "ffconf.h: _USE_WRITE must be 1 — the library index writes /dap.idx"
#endif

#if _USE_LFN == 0
#error "ffconf.h: _USE_LFN must be >= 1, or every artist and album indexes as an 8.3 name"
#endif

/* The scan holds one lock slot per open file plus one per nested
 * directory level. Too few and f_opendir returns FR_TOO_MANY_OPEN_FILES
 * one level down, the subtree is skipped, and the card looks empty. */
#if _FS_LOCK != 0 && _FS_LOCK < (LIB_BUILD_MAX_DEPTH + 4)
#error "ffconf.h: _FS_LOCK is too small for the scan — needs LIB_BUILD_MAX_DEPTH + 4, or 0 to disable locking"
#endif
